...
- FR-8b.1 The intelligence features run on-device by default. On-device means local heuristics, not a local model: keyword tag hints from a note's own words, and a truncated head of a long note in place of a summary. **Nothing that requires understanding the note** — no transcription, no reading an image, and no matching by meaning. Each of those reports itself unavailable (FR-8b.4) rather than answering with something shaped like an answer. *The last of the three was not always true: the on-device adapter used to return an embedding derived from a hash of the text, which has no similarity structure at all, and search offered the result under its own "matches by meaning" heading. A number that looks like a score is the hardest kind of wrong answer for a user to catch.*
...
- FR-8b.5 The key is stored in the device's **secure storage** — the platform keystore, not the app's ordinary preferences — and what the UI says about it must be what the code does. It is sent to the chosen provider and nowhere else. *This clause used to require the opposite. It read "stored in the app's private preferences… **not encrypted**, and this is stated plainly in the UI", which was true of the first implementation: on Android `shared_preferences` is an unencrypted XML file readable on a rooted device or through an ADB backup, and on Windows it is plaintext JSON. The code moved to `flutter_secure_storage` (keys namespaced per provider, with a one-time migration out of the plaintext slot) and the UI was updated to say so; only this line stayed behind, leaving the specification asserting a weakness the app no longer has. The requirement that survives the change is the one the original was written for: the disclosure and the storage have to agree.*
