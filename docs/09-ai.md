...
- Lives in `packages/ai`, not Core — the one package the [architectural
  boundary](#architectural-boundary) above already guarantees is deletable.
  `apps/client` had never depended on it before this; it does now, but only
  the dev harness below imports it.
...
- A throwaway measurement harness, not a preview of the real feature:
  `apps/client/lib/dev/local_ai_bench_main.dart` (standalone entry point,
  unreachable from `main.dart`) plus `apps/client/lib/dev/README.md` and
  `apps/client/android/app/libs/README.md` for the manual one-time setup
  (native library, a `.gguf` model) it needs. Answers one question — tokens/
  sec, time-to-first-token, peak memory on real hardware — before anything
