...
which fits under the cap as a single asset and skips the split entirely. Nothing
in `NexModelStore` distinguishes a one-part release from a two-part one, so this
is a change to a constant and not to the download path. Two things keep it from
being the automatic choice:

- **Its Persian quality is unverified.** The seven prompts that decided Gemma-4
  was good enough were run against the general build in Edge Gallery, not this
  one. Those prompts are the gate, and they have to be re-run before this
  becomes the shipped model.
- **It is a backend-specific build.** `LiteRtChatAdapter` tries GPU and falls
  back to CPU precisely so a device with no working OpenCL path still gets an
  answer. A GPU-targeted artifact may refuse to load on the CPU backend, which
  would turn that fallback into a 2 GB download that fails at the end — the
  exact outcome `local_ai_support.dart` exists to prevent. Whether it does is a
  question for the first install on real hardware, not for a doc.
