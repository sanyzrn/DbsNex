...
1. **Swap the dependency.** `llama_cpp_dart` out of `packages/ai`,
   `flutter_gemma` in. The Phase 0 pin and the AAR instructions go with it, and
   `experiments/local_ai_bench` becomes dead weight rather than a harness.
2. **A real `ChatAdapter`** over `flutter_gemma`, replacing
   `PlaceholderLocalChatAdapter`: load, generate, cancel, unload, and the
   scope ceiling already written.
3. **Model file management** — the largest piece. A 2.6 GB download with
   resume, integrity verification, storage accounting, and deletion. The
   in-app updater already solves most of this shape for APKs and is the place
   to look first rather than a second implementation.
4. **Device gating.** arm64-v8a, OpenCL present, enough free storage. A device
   that fails any of these never sees the feature offered, the way
   `aiCapabilities` already models "off".
5. **The UI** for all of the above, and binding it in `main_ai.dart`.

**The open decision, which is not an implementation detail: where the 2.6 GB
comes from.** Cafe Bazaar is the distribution target, so Hugging Face is not a
dependency that can be assumed to work for the people who would use this. The
options are to mirror the weights somewhere reachable and pay for that
bandwidth, or to let people supply the file themselves — which the app can
already almost do, since `NoteImportArchive` established the pattern of taking
a picked file and doing something with it. That choice is upstream of the UI in
item 5 and should be made before it, not during it.
