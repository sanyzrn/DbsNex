...
The constant still points at the `-gpu` file for now, deliberately. Splitting
and uploading 2.58 GB is an hour of someone's evening, and it is the wrong hour
to spend while the cause is a guess. Three things produce the same red line and
point different ways: the file may be the wrong format for whichever backend
loads it, the phone may have no working OpenCL path, or 2 GB of weights may not
fit in memory beside the app. Only the first is fixed by uploading the other
file.

So the load error is kept and shown verbatim — under the installed state on the
model screen, selectable so it can be quoted, and in the chat in place of "no
answer came back, check the provider in Settings", which is the same sentence
for all three causes and is wrong advice for someone who chose to have no
provider. One run settles which of the three it is.
