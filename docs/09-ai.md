...
**Open question this doc does not yet answer:** how a real on-device
implementation gets bound into a shippable app at all, given `apps/client`
must never depend on `nex_ai` (Phase 0, above). Two shapes were considered
but not decided: (a) local chat ships as its own separate app/build target
(alongside, not instead of, `apps/client`) that bundles `nex_core` +
`nex_ai` — consistent with Cafe Bazaar/Myket already being a distinct
distribution target from `apps/client`'s eventual Play track; (b) an
Android build-flavor split within `apps/client` itself, which would need
the `ai-deletion-proof` CI assertion narrowed to the AI-free flavor rather
than the whole `pubspec.yaml`. Needs a decision before the chat UI has
somewhere real to live — until then, `ChatAdapterBinding` defaults to
`NullChatAdapter` and nothing in `apps/client` binds anything to it,
exactly like `AIAdapterBinding` behaves with AI off today.
