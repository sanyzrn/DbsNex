...
**One constraint shapes the work: GitHub caps a release asset at 2 GiB**, and
the model is 2.6 GB. It ships **split across two assets** (a release accepts up
to 1000), with the app fetching both, verifying SHA-256 per part and over the
joined file, and concatenating. `app_update.dart` already does resumable
download with SHA-256 verification, so joining parts is the only genuinely new
piece — this reuses a path that already works rather than building a second
downloader.
