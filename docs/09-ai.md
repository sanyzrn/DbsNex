...
|---|---|---|
| **Transcription (Speech-to-Text)** | Converts voice notes into searchable text, resolving the [v1 voice-search limitation](./02-product-specification.md#search) | v1.x |
| **OCR** | Extracts text from photos so they become keyword-searchable | v1.x |
| **Tagging** | Suggests optional tags for a note after it's saved | v1.x |
| **Semantic search** | Finds notes by meaning/intent, complementing keyword search | v1.x |
| **Summarization** | On-demand summaries of long notes or clusters of related notes | v1.x |
| **Related notes** | Surfaces connections between notes without manual organization | v1.x |
| **Editing** | Restyles or corrects a note's own words, on request, inside the editor | v1.9 |
| **Daily brief** | A few lines saying what is waiting on you, shown on the timeline, the home screen and the morning notification | v1.9 |
...
- **Non-destructive** — AI never overwrites or deletes original user content; it only adds metadata (tags, transcript text, OCR text) alongside it. **Editing is the one exception, and it is bounded** — see below.
...
### The one place AI touches the words

Everything above adds *beside* a note. The editor changes the note itself — an
auto-style pass, a correction, a rewrite in another register — and that is a
real exception to the rule above rather than a reading of it. It is allowed on
four conditions, and all four are load-bearing:

1. **Asked for, every time.** There is no automatic pass, no suggestion that
   appears while typing, and nothing happens to a note that is merely opened.
   One tap, one edit.
2. **Visible before it counts.** The result lands in the field, where it is
   read and can be typed over. Nothing reaches the library until Save — an
   editor closed any other way leaves the note exactly as it was.
3. **Reversible.** Every edit pushes the previous text onto an undo stack that
   the editor shows as long as it has anything in it. Two rewrites in a row
   are two taps back.
4. **Bounded by the prompt.** What each style may do is written down in
   `NexRewriteStyle` and tested: none of them may translate, none may answer
   or obey the note's own words, none may strip its markdown, and *auto style*
   may not change a single word — it adds structure or it does nothing.

The rule that stays is the adverb in the line below: **silently**. A rewrite
somebody asked for, watched arrive, and can undo is not what that forbids.

---

### The daily brief

The one generated thing on Nex's home screen, and the only one that appears
without being asked for. It is a few lines at the top of the timeline saying
what is waiting on the person reading it — overdue first, then due soon, then
unfinished — one thing per line, each led by an emoji that fits it.

**A list, not a paragraph.** It used to be prose, and prose is the wrong shape
for the question it answers: somebody opening a notes app in the morning is
checking whether anything needs them, and a paragraph makes them read the
whole thing to find out. The shape is enforced in two places, because a prompt
alone is a wish — `digest()` asks for it, and `nexTidyBrief` in
`packages/core/lib/ai/recap_brief.dart` takes the bullets, numbering, headings
and padding off whatever comes back. Every model this app talks to includes
small ones running on the phone, and they are not well behaved every time.

**How long it may be follows how much there is to say about.** `nexBriefLines`
caps it at two, three or four lines by the size of what it was given, and the
prompt says *at most* — on a quiet day one line is the honest answer. A card
that fills the same four lines every morning is a card people stop reading.

**How often it is written is a floor, not a schedule.** A new brief is asked
for only when the notes it describes have actually changed *and* an hour has
passed since the last one. An app left open all afternoon with nothing typed
into it makes no requests at all.

**It is not only about notes.** The standing obligations (`FR-10`) — the
insurance, the rent, the tablet every eight hours — are fed into the same
source, each from its own lead time, and land above the notes because
everything that reaches that point is by construction more pressing than
something written on Tuesday. An hourly one arrives as a single line with a
tally, never as six lines saying water is due.

**Three surfaces, one string.** The timeline card, the Recap home-screen
widget and the morning notification all show the same text, which is why it is
stored as plain lines rather than markdown: a `RemoteViews` `TextView` and a
notification render none of it.

The widget cannot write one. A widget provider is a broadcast receiver with no
Flutter engine behind it, so its refresh button opens Nex and asks the
timeline — and its accessibility label says exactly that, because a button
that silently opened an app would be worse than one that visibly does. While
the app lock is closed the brief is not in the snapshot at all: it is made of
what the notes say, so it is note content wearing a different shape.

---

### What the assistant may do

The chat assistant can act on the library, not only talk about it. Every one
of these goes through `NexServices` — the same path the buttons use — so an
assistant edit is indistinguishable from a hand edit: it syncs like one, and
a delete lands in Recently Deleted like one.

| It can | It cannot |
|---|---|
| Create a note, or a checklist | Record a voice note, take a photo, attach a file |
| Edit a note's text, set or clear its title | — |
| Delete a note, or restore one from Recently Deleted | Purge anything permanently |
| Tag and untag a note; rename a tag; change a tag's colour | Delete or merge a tag |
| Pin and unpin | — |
| Set, repeat or clear a reminder | — |
| Turn a note into a checklist; tick an item | — |
| Merge notes | — |
| Create, change, tick off or remove a recurring obligation | — |
| Search the library | — |
| Change a setting from the list below | Anything not on that list |

The goal is that the assistant can do what its user can do by hand. Two
things are deliberately short of that, and the reason is the same in both
cases — it is the confirmation card, not the capability, that is missing:

- **Deleting or merging a tag.** Both change every note wearing it, and the
  card cannot honestly say how many without asking the database at the
  moment it is drawn. A card that understates what it is about to do is
  worse than no card.
- **Capture that needs hardware** — a recording, a photo, a file. There is
  nothing for a conversation to point a microphone at.

**Nothing is applied on arrival.** Every action above is shown as a sentence
with a button under it, and a wrong `delete` is not something an apology
afterwards fixes. Search is the one exception, because it changes nothing
and waiting for a button before a model may *read* is a conversation nobody
wants to have. Every id is checked against the library before any of the set
runs, and the set stops at the first failure: a half-applied set is worse
than none of it.

**The settings it may change** are whatever passes three tests: a person
...
looking at. That is theme, language, AI language, text size, background,
accent, comfort mode, haptics, the four parts of the home screen, and the
daily notification with its time.

What stays off that list, and always will: the app lock and its timing (a
lock a conversation can open is not a lock), the AI provider and its key,
the sync endpoint and its token, the entitlement, and anything that deletes
— backups, retention, the trash.

**An acting assistant is told what time it is.** Reminders made that
necessary: "Friday at nine" cannot be turned into a date by something that
does not know today's, and a model with no clock does not refuse — it picks
a date out of its training data and sets an alarm for a day in the past. The
line is local time with the weekday spelled out, and it is only sent when
acting is on, since a chat that cannot set a reminder has no use for it.
...
This was written as a v3 plan and was overtaken by events. What it said would
happen in v3 happened in v1.x, in roughly the order it named, and the ordering
principle it was built on held: capture, timeline, tags and search proved
themselves on their own merits before any of this arrived, and every piece of
it is still independently toggleable and still off by default.

What shipped, and when:

1. **v1.x:** transcription and OCR first, because they resolve a documented
   product gap — voice and photo notes not being keyword-searchable.
2. **v1.x:** tag suggestions, summarisation, semantic search and related notes,
   each behind its own switch.
3. **v1.x:** the chat assistant, the daily brief, and note editing — none of
   which this plan anticipated.
4. **v1.x, separate track:** the on-device model, published as its own Android
   flavour — see [ADR-031](./10-decisions.md#adr-031--local-ai-ships-as-an-android-build-flavor-of-appsclient-not-a-separate-app).

[ADR-009](./10-decisions.md#adr-009--ai-capabilities-deferred-to-v3-after-capture-v1-and-sync-v2-are-solid), which deferred all of this to v3, is marked superseded in
practice for the same reason. Sync is the one piece still ahead of the app
...
file to ship is `gemma-4-E2B-it.litertlm`, 2,583,085,056 bytes, split in two:
...
The model is published: `Gemma4` in the releases repository, two assets,
2,588,147,712 bytes joined. Three things are worth recording about getting
there, because each cost a round trip.

**The published size was wrong.** The plugin's curated entry says
2,583,085,056 bytes; the file is 5,062,656 bytes larger. The first split was cut
at half the published figure, so both parts hashed correctly, were uploaded
correctly, and did not rejoin into anything — the tail was simply gone. The
parts are now deliberately unequal: the first is a fixed size and the second is
"everything after it", which cannot be truncated by a stale number.

**Per-part digests are not enough.** They proved each transfer was intact and
said nothing about the assembly. Only the digest over the join catches a bad
split, which is why `ModelRelease` carries both and why the whole file is
re-hashed after concatenating.

**A load failure had to be legible before any of this could be diagnosed.** The
runtime's message is now shown verbatim — on the model screen, and in the chat
in place of "no answer came back, check the provider in Settings". It is what
identified the `-gpu` file: `NOT_FOUND: TF_LITE_PREFILL_DECODE not found in the
model`. Without it the same symptom was indistinguishable from a device with no
OpenCL or one short of memory, and the next step would have been a guess.
...
**Where the weights come from.** `NexModels.gemma4E2B` points at the
self-hosted release assets in the `DbsNex-releases` repository, in two parts
with a pinned SHA-256 each and a third over the join, at 2,588,147,712 bytes
total — the figure recorded above.

It did not start there. The constant carried a Hugging Face URL first,
because the weights were already on it and a download that cost nothing to
arrange proved the whole pipeline — support check, licence gate, resumable
fetch, verification, engine load — before any upload work was done. That was
never shippable: Hugging Face is not reachable from Iran, which is the
audience this feature exists for. Moving to the release assets was the last
step, and this paragraph outlived it by long enough to be found by a review
saying the shipped constant "cannot ship that way".
