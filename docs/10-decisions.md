...
- **Status:** Accepted, with one clarification. The decision — two separate numbers for two separate audiences — stands and is the right one. "Enforced in CI" is true of two of the budgets, by tests inside the suites CI already runs rather than by a job named for them; it was never true of the two device-bound ones, which need a real phone and are validated by usability testing instead. That distinction had not been written down, and the absence of a named job made the enforcement look absent to two separate readers who went looking in the workflow files. [`02-product-specification.md`](./02-product-specification.md#non-functional-requirements) now says which is which, where each one lives, and why the other two cannot be gated honestly.
...
- **Status:** Accepted, amended at v0.9.6. See [`09-ai.md`](./09-ai.md#phase-1--local-chat-core-free) for the flavor/entry-point details. Two things this ADR originally wrote down have since changed, both because the placeholder became a real model:
  - **The `applicationIdSuffix` is gone.** `".ai"` let both flavors sit on one phone, which is right for development and wrong for shipping. `"ai"` is the flavor people install, so a suffix made it a *different app* from the one already on their phone — none of their notes, and an in-app updater offering an APK Android refuses to install across applicationIds, which is the v0.9.1 install failure by another route. Side-by-side is recoverable from the `debug` build type if anything ever needs it.
  - **`release.yml` publishes `"ai"`, not `"standard"`.** From v0.9.6 the Android bundle and APKs are the `"ai"` flavor built against `lib/main_ai.dart`. `"standard"` is not abandoned: CI still builds it, and the `ai-deletion-proof` job still guarantees it survives `packages/ai` being deleted. It is simply not what gets published while the on-device model is the reason to install the app at all. A Play listing would bring it back.
  - The flavor is also built now — `ai-testbuild.yml` produces an installable APK on demand, which is what the last line of this ADR deferred.
