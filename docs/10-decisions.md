...
## ADR-022 — Configurable swipe actions, limited to a fixed two-action set

- **Context:** A request was made for swipe gestures on Timeline cards — swiping one direction triggers one action (e.g., delete), the other direction a different action — with the direction-to-action mapping configurable by the user (e.g., to match left- vs. right-hand swiping habits).
- **Decision:** Each Timeline card supports exactly two swipe-revealed actions: **Delete** (soft-delete, undoable via toast) and **Add Tag** (opens inline tag input). Which edge (leading/trailing) triggers which action is a user-configurable preference, stored locally as a simple key-value setting, reachable from a new lightweight Settings sheet (opened by tapping the avatar). The set of possible actions itself is fixed at these two — it is not an open or extensible action framework, and it deliberately excludes actions that would reintroduce organization Nex has already ruled out (e.g., **pinning is not offered as a swipe action**, consistent with the Timeline's existing "no pinning, no manual reordering" rule).
- **Rationale:** Letting the person choose *which side* does *what* costs nothing structurally (it's a two-value local preference) and respects a genuinely personal ergonomic difference. But an open-ended, arbitrary action list would violate the project's standing rule against speculative configurability (see [`06-development.md`](./06-development.md#coding-principles), principle 3: "No speculative abstraction") and would need its own settings sub-screen, working against "no settings-heavy home screen." Keeping the action set closed at two — one non-destructive (Add Tag), one destructive-but-reversible (Delete) — gets the ergonomic benefit the person asked for without opening that door.
- **Alternatives Considered:** (1) A fully open, user-customizable action list (assign any of N actions to either swipe direction) — rejected as scope creep disproportionate to the benefit, and as the first crack in Nex's "small, fixed surface" identity. (2) Fixed, non-configurable swipe directions — rejected because the person specifically wants to choose the mapping, and honoring that costs almost nothing. (3) Adding Pin/Archive as a third swipe-able action — rejected outright, since Nex has already decided against pinning and manual reordering in the Timeline; adding it here would quietly reverse that decision through a side door.
- **Status:** Accepted, planned for v1.x (ships alongside the polish/stability pass, not required for the v1.0 MVP exit criteria — it strengthens Organize Later and Find Instantly but isn't necessary to prove the two core capture/find promises). See [`02-product-specification.md`](./02-product-specification.md#fr-2--timeline) and [`05-design.md`](./05-design.md#swipe-actions).
...
- **Status:** Accepted, planned for v1.x (ships in the same Settings sheet introduced for [ADR-022](#adr-022--configurable-swipe-actions-limited-to-a-fixed-two-action-set)'s swipe-action mapping). See [`05-design.md`](./05-design.md#comfort-mode) for the token specification.

---

## ADR-024 — Flutter as the single cross-platform client framework

- **Context:** The original tech-stack recommendation split the client into two separate stacks sharing a JS/TypeScript "core" package: React Native for Android (and future iOS), and Tauri or Electron for Windows desktop. Revisiting this choice directly, given the product's actual requirements — a solo/small team building for Android first, Windows second, iOS later, all sync-connected, offline-first, with custom gestures (swipe actions, [ADR-022](#adr-022--configurable-swipe-actions-limited-to-a-fixed-two-action-set)), theming (Light/Dark/Comfort, [ADR-023](#adr-023--comfort-mode-as-an-independent-axis-from-lightdark-theme)), and mixed LTR/RTL text ([`05-design.md`](./05-design.md#tag-accent-color)) — surfaced a simpler option.
...
- **Status:** Accepted. Supersedes the client portion of the original stack recommendation in [`03-readme.md`](./03-readme.md#tech-stack); the backend recommendation is unchanged.
