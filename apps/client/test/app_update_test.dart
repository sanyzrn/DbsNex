...
void main() {
  ...
  group('UpdateChecker.listReleases', () {
    test('returns every release, newest first, as published', () async {
      final checker = UpdateChecker(
        currentVersion: '0.2.0',
        client: MockClient(
      ...
      final releases = await checker.listReleases();

      expect(releases, hasLength(2));
      expect(releases[0].version, '0.3.0');
      expect(releases[0].notes, '- newest release');
      expect(releases[1].version, '0.2.0');
    });

    test('skips drafts and releases with no body at all', () async {
      final checker = UpdateChecker(
        currentVersion: '0.2.0',
        client: MockClient(
          (_) async => http.Response(
            jsonEncode([
              {'tag_name': 'v0.4.0', 'draft': true, 'body': '- unreleased'},
              {'tag_name': 'v0.3.0', 'draft': false, 'body': null},
              {'tag_name': 'v0.2.0', 'draft': false, 'body': '- kept'},
            ]),
            200,
          ),
        ),
      ...
      expect(releases.single.version, '0.2.0');
    });

    test('a failed fetch is an empty list, not a crash', () async {
      final checker = UpdateChecker(
        currentVersion: '0.2.0',
      ...
      expect(await checker.listReleases(), isEmpty);
    });
  });
