...
Phase 0 of the offline-AI roadmap (`docs/09-ai.md`) needs the native
llama.cpp runtime that `lib/dev/local_ai_bench_main.dart` loads through the
`llama_cpp_dart` package. That package ships **no native binaries** in its
Dart package — they're distributed separately as prebuilt release assets.

To try the bench harness on a device:
...
1. Go to <https://github.com/netdur/llama_cpp_dart/releases> and grab
   ...
   pinned version in `packages/ai/pubspec.yaml` (`0.9.0-dev.10` at the time
   this was written — a newer release is probably fine too, but re-check the
   pubspec pin first).
...
3. Build/run as usual. `android/app/build.gradle.kts` only adds it as a
   dependency if this file exists — its absence never breaks the normal
   app build, which is why it isn't committed and why CI doesn't need it.
